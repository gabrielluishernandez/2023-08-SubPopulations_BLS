...
# Save the plot to the current working directory
ggsave("results/ncd_plot_400.png", plot = ncd_plot_400_no_heparin)
...
#Top 1 percent of the distributions 
quantile(from_apocrita_400$mean_density, 0.01)
quantile(from_apocrita_400$density, 0.01)
```
...
##  geom_smooth(method = "lm", se = TRUE, color = "grey") +  
  ...
  #guides(shape = FALSE) +
  geom_hline(yintercept = 0.01092523, linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = 0.00477119, linetype = "dashed", size = 0.5) +
  theme_minimal()
density_plot_400
...
ggplot(from_apocrita_400 %>% subset(gene != "LOC105199771") 
                       %>% arrange(candens), 
       aes(x = density, y = mean_density, colour = candens)) +
  geom_point(size = 3, alpha = 0.6) +
  # Adding linear regression line
  geom_smooth(method = "lm", se = TRUE, color = "grey") +  
  # Add names to selected points
  geom_label_repel(data = three_genes, aes(label = gene), colour = "black",
                   size= 2, direction = "both",
                   box.padding = 0.3, max.overlaps = 50) +
  # Change colors
  scale_colour_viridis_d(option = "C", direction = -1, 
                         name = "Candidate", labels = c("No","Yes")) + 
  #Change axis labels
  labs(y = "combined mean 400 subpops", x = "combined all samples") + 
  #guides(shape = FALSE) +
  theme_minimal()
...
# Save the plot to the current working directory
ggsave("results/density_plot_400.png", plot = density_plot_400)
